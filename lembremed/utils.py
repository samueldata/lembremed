...
def rodar_tg_bot():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            DESCADASTRADOS_ESCOLHENDO: [
                MessageHandler(
                    filters.Regex("^(Cadastrar)$"), opcao_descadastrados
                ),
                MessageHandler(
                    filters.Regex("^(Sair)$"), sair
                ),
            ],
            CADASTRADOS_ESCOLHENDO: [
                MessageHandler(
                    filters.Regex("^(Descadastrar|Listar remédios)$"), opcao_cadastrados
                ),
                MessageHandler(
                    filters.Regex("^(Sair)$"), sair
                ),
            ...
            DIGITANDO_SENHA: [
                MessageHandler(
                    filters.Regex("^(Confirmar cadastro|Voltar)$"), receber_senha
                ),
            ],
        },
        fallbacks=[
            MessageHandler(
                filters.ALL, mensagem_estranha
            ),
            CommandHandler(
                'cancel', sair
            ),
    ...
    application.add_handler(CallbackQueryHandler(apertar_botao))
